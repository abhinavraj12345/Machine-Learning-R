...
trainData=Da[,-1]
up_train=downSample(x=trainData[,-ncol(trainData)],y=trainData$Loan_Status,yname ="Loan_Status" )
noData=up_train[1:(nrow(up_train)/2),]
yesData=up_train[-c(1:(nrow(up_train)/2)),]
#------------------------test data---------------------------------------------------
...
CV.RF.ntree.mtry<-function(yesdata,noData,fold,a,b){ #other function parameters){
  d=floor(nrow(yesdata)/fold)
  k=1:d
  train<-NULL
  test<-NULL
  MCC=NULL
  for(i in 1:fold){
    test=rbind(yesData[k,],noData[k,])
    train=rbind(yesData[-k,],noData[k,])
...
for(i in 1:length(x)){
  for(j in 1:length(y)){
    aa[i,j]=CV.RF.ntree.mtry(yesData,noData,5,x[i],y[j])
  }#END:j
}#END:i
...
#----------------------------------------------------------------------------------------
finaltree=randomForest(formula=Loan_Status~.,data=trainData,mtry=x[l[1,2]],ntree=y[l[1,1]])
